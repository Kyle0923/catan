 ...
 * '''
 *  4. add a case statement and use the extractValue<CliOptIndex> utility to extract values.
 *     for Boolean flags, if you do not care about the value user passes in,
 *     you can directly set it to true
 * '''
 *      case CliOptIndex::DEBUG_LEVEL:
 *          extractValue<CliOptIndex::DEBUG_LEVEL>(argc, argv, ii);
 *          break;
...
public:
    ...
    using TYPE_AT = typename std::tuple_element<Index, CliOptList_t >::type;
    ...
    int processArg(const int argc, const char* const *argv)
    {
        for (int ii = 0; ii < argc; ++ii)
        {
            for (size_t optIndex = 0U; optIndex < CliOptIndex::CLI_OPT_INDEX_END; ++optIndex)
            {
                ...
                {
                    // extractValue utility to extra values from argv
                    // e.g., extractValue<CliOptIndex::DEBUG_LEVEL>(argc, argv, ii)
                    switch (optIndex)
                    {
                    case CliOptIndex::DEBUG_LEVEL:
                        extractValue<CliOptIndex::DEBUG_LEVEL>(argc, argv, ii);
                    ...
                    case CliOptIndex::MAP_FILE_PATH:
                        extractValue<CliOptIndex::MAP_FILE_PATH>(argc, argv, ii);
                        break;
                    default:
...
private:
    ...
    template<CliOptIndex optIndex, typename T = TYPE_AT<optIndex>,
    typename std::enable_if_t<std::is_integral<T>::value && ! std::is_same<T, bool>::value>* = nullptr >
    int extractValue(const int argc, const char* const *argv, int& aIndex)
    {
        return extractValueImpl<optIndex>(argc, argv, aIndex, \
            [](const char* str, const char** endptr) {
                return static_cast< TYPE_AT<optIndex> >(std::strtol(str, const_cast<char**>(endptr), 10));
            });
    }
    ...
    int extractValue(const int argc, const char* const *argv, int& aIndex)
    {
        return extractValueImpl<optIndex>(argc, argv, aIndex, \
            [](const char* str, const char** endptr) {
                return static_cast< TYPE_AT<optIndex> >(std::strtod(str, const_cast<char**>(endptr)));
            });
    }

    template<CliOptIndex optIndex, typename T = TYPE_AT<optIndex>,
    typename std::enable_if_t<std::is_same<T, typename std::string>::value>* = nullptr >
    int extractValue(const int argc, const char* const *argv, int& aIndex)
    {
        return extractValueImpl<optIndex>(argc, argv, aIndex, \
            [](const char* str, const char** endptr) {
    ...
    int extractValue(const int argc, const char* const *argv, int& aIndex)
    {
        int rc = extractValue<optIndex, int>(argc, argv, aIndex);
        if (rc == 1)
        {
    ...
    int extractValueImpl(const int argc, const char* const *argv, int& aIndex, TYPE_AT<optIndex> (*aStrToValue)(const char* str, const char** endptr))
    {
        int rc = 0;
        ...
        const TYPE_AT<optIndex> typedValue = static_cast< TYPE_AT<optIndex> >(aStrToValue(value, &endptr));
